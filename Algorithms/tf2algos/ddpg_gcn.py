...
class DDPG_GCN(Off_Policy):
    def __init__(self,
        ...
        if self.is_continuous:
            self.actor_net = Nn.actor_dpg_gcn([10,10],[10,6], self.a_counts, 'actor_net', hidden_units['actor_continuous'], visual_net=self.actor_visual_net)
            self.actor_target_net = Nn.actor_dpg_gcn([10,10],[10,6], self.a_counts, 'actor_target_net', hidden_units['actor_continuous'], visual_net=self.actor_visual_net)
        ...
        self.q_net = Nn.critic_q_one_gcn([10,10],[10,6], self.a_counts, 'q_net', hidden_units['q'], visual_net=self.critic_visual_net)
        self.q_target_net = Nn.critic_q_one_gcn([10,10],[10,6], self.a_counts, 'q_target_net', hidden_units['q'], visual_net=self.critic_visual_net)
...
　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　ｘｘ　　　
    ...
    def choose_action(self, adj, x, evaluation=False):
        a = self._get_action(adj, x, self.vec, evaluation).numpy()
        return a if self.is_continuous else sth.int2action_index(a, self.a_dim_or_list)

    @tf.function
    def _get_action(self, adj, x, vec, evaluation):
        # s, visual_s = self.cast(s, visual_s)
        with tf.device(self.device):
            if self.is_continuous:
                mu = self.actor_net(adj, x, vec)
                pi = tf.clip_by_value(mu + self.action_noise(), -1, 1)
            else:
                logits = self.actor_net(adj, x, vec)
    ...
    def learn(self, **kwargs):
        ...
        for i in range(kwargs['step']):
            if self.data.is_lg_batch_size:
                adj, x, a, r, adj_, x_, done = self.data.sample()
                if self.use_priority:
                    self.IS_w = self.data.get_IS_w()
                td_error, summaries = self.train(adj, x, a, r, adj_, x_, done)
                if self.use_priority:
    ...
    def train(self, adj, x, a, r, adj_, x_, done):
        adj, x, a, r, adj_, x_, done = self.cast(adj, x, a, r, adj_, x_, done)
        with tf.device(self.device):
            with tf.GradientTape() as tape:
                if self.is_continuous:
                    target_mu = self.actor_target_net(adj_, x_, self.vec)
                    action_target = tf.clip_by_value(target_mu + self.action_noise(), -1, 1)
                else:
                    target_logits = self.actor_target_net(adj_, x_, self.vec)
                ...
                q = self.q_net(adj, x, self.vec, a)
                q_target = self.q_target_net(adj_, x_, self.vec, a, action_target)
            ...
            with tf.GradientTape() as tape:
                if self.is_continuous:
                    mu = self.actor_net(adj, x, self.vec)
                    pi = tf.clip_by_value(mu + self.action_noise(), -1, 1)
                else:
                    logits = self.actor_net(adj, x, self.vec)
                    ...
                    pi = _pi_diff + _pi
                q_actor = self.q_net(adj, x, self.vec, pi)
    ...
    @tf.function(experimental_relax_shapes=True)
    def train_persistent(self, adj, x, a, r, adj_, x_, done):
        adj, x, a, r, adj_, x_, done = self.cast(adj, x, a, r, adj_, x_, done)
        with tf.device(self.device):
            with tf.GradientTape(persistent=True) as tape:
                if self.is_continuous:
                    ...
                    pi = _pi_diff + _pi
                q = self.q_net(adj, x, self.vec, a)
                q_target = self.q_target_net(adj_, x_, self.vec, action_target)
                dc_r = tf.stop_gradient(r + self.gamma * q_target * (1 - done))
                td_error = q - dc_r
                q_loss = 0.5 * tf.reduce_mean(tf.square(td_error) * self.IS_w)
